...
- [x] In `textkit/core.py` add a module-level constant near the top:
  `TITLE_CASE_CONNECTORS = frozenset({"a", "an", "the", "and", "or", "of", "in", "on"})`.
- [x] Add `title_case(text: str) -> str` (place it after `shout`, keeping the
...
- [x] Export it: add `title_case` to both the `from .core import (...)` list
  and `__all__` in `textkit/__init__.py`, alphabetically between `slugify`
  and `truncate`.
- [x] Standard library only; no new imports needed.

## Task 2: unit tests for `title_case`

- [x] In `tests/test_core.py` add `title_case` to the `from textkit import (...)`
...
- [x] No changes to `handle_transform`, routing, `RESULT_PLACEHOLDER`, the
...
- [x] `tests/test_web.py`, existing tests that must be updated:
...
- [x] `tests/test_web.py`, new assertions:
...
- [x] `e2e/tests/footer.spec.js`: `FOOTER_TEXT` → `"textkit playground · 6 operations"`.
- [x] `e2e/tests/clear-button.spec.js` (not in the plan, found during execution):
  `test("clear button is rendered next to apply…")` pins every button inside
  `p:has(#apply)` — expected list becomes `["Apply", "Clear", "Title Case"]`.
- [x] `e2e/tests/playground.spec.js`: add `title_case: "Ada Lovelace Was a Mathematician"`
...
- [x] New `e2e/tests/title-case.spec.js`, header comment pointing at
...
- [x] Follow the `changelog` skill: append **one** line to the end of
  `CHANGELOG.md`:
...
- [x] `python3 -m unittest discover -s tests -v` — 75 green
  (65 existing + 8 core + 2 web).
- [x] Manual/e2e check: `TEXTKIT_PORT=3000 python3 -m textkit.web`, then
...
- [x] Playwright suite from `e2e/` against a running server on port 3000
  (`E2E_BASE_URL=http://localhost:3000 npx playwright test`) — 37 passed.
  Needs `npm ci` plus
  `PLAYWRIGHT_BROWSERS_PATH=/home/sandbox/.cache/ms-playwright npx playwright install chromium`
  first: the default `/opt/pw-browsers` is read-only and `--with-deps` needs root.
- [x] Confirm no `__pycache__/` or `*.pyc` is staged. Initially committed per
  task; squashed into the single planned commit after review.
  Not pushed.
...
| `e2e/tests/clear-button.spec.js` | button list gains `Title Case` |
